...
# SimHash is optional — only imported when use_simhash is True
...
class Trainer(object):
    ...
    def collect_seed_episodes(self, env):
        s = env.reset()                          # gym: returns obs only
        done = False
        for i in range(self.seed_steps):
            a = env.action_space.sample()
            ns, r, done, _ = env.step(a)         # gym: 4-tuple
            if done:
    ...
    def train_batch(self, train_metrics):
        """
        trains the world model and imagination actor and critic for collect_interval
        times using sequence-batch data from buffer
        """
        actor_l = []
        value_l = []
        obs_l = []
    ...
    def representation_loss(self, obs, actions, rewards, nonterms):
        ...
        # --- SimHash intrinsic reward (no-op when use_simhash=False) ---
    ...
    def get_save_dict(self):
        ...
        if self.config.use_simhash:
            d['SimHashModel']  = self.SimHashModel.state_dict()
            d['SimHashCounts'] = self.SimHashModel.get_counts()
        return d
    ...
    def load_save_dict(self, saved_dict):
        ...
        if self.config.use_simhash and 'SimHashModel' in saved_dict:
            self.SimHashModel.load_state_dict(saved_dict['SimHashModel'])
            self.SimHashModel.set_counts(saved_dict.get('SimHashCounts', {}))
    ...
    def _model_initialize(self, config):
        ...
        self.buffer = TransitionBuffer(
            config.capacity, obs_shape, action_size, config.seq_len,
            config.batch_size, config.obs_dtype, config.action_dtype,
        )
        self.RSSM = RSSM(
            action_size, rssm_node_size, embedding_size, self.device,
            config.rssm_type, config.rssm_info,
        ).to(self.device)
        self.ActionModel = DiscreteActionModel(
            action_size, deter_size, stoch_size, embedding_size,
            config.actor, config.expl,
        ).to(self.device)
